...
## Aggiornamento 09/06/2026 — Esperienza per sede, richiesta tavolo, assistente AI (da video di riferimento dell'utente)
- Scelta iniziale sede ("Dove sei, a Málaga o a Malta?") salvata in `localStorage.kalama_site`; hero con badge sede, sottotitolo per sede e badge delivery (Glovo/Bolt Food); il menu si preseleziona sulla sede; cambio sede da navbar (desktop) e drawer.
- Drawer laterale (hamburger, portal su body) con link, cambio sede, Richiedi un tavolo, Come arrivare, WhatsApp/Instagram/Chiama. Barra fissa mobile "Come arrivare / Richiedi un tavolo". Banner privacy (`kalama_privacy_ok`).
- Richiesta tavolo in 4 passi (data 14 giorni, pranzo/cena con fasce da `/api/table-requests/slots` — Málaga chiuso lunedì —, coperti; zona+occasione+accesso; dati; riepilogo): `POST /api/table-requests` salva in `table_requests`, invia email alla sede (Málaga info@kalama.eu, Sliema kalama.international@gmail.com) e restituisce link WhatsApp precompilato. Tab admin "Prenotazioni" (stato nuova/confermata/rifiutata, elimina).
- Assistente AI "Kalamà ti aiuta": GPT-5.4 mini via EMERGENT_LLM_KEY (emergentintegrations), streaming SSE `POST /api/chat`, cronologia in `chat_messages` per `session_id` (localStorage `kalama_chat_session`), system prompt con menù della sede, orari, delivery; quick actions (menu, tavolo, WhatsApp, mappa). Logica in `/app/backend/booking.py`.
